  ...
  def self._get_list(xmlfile, xpath, cls)
    ...
    doc.xpath(xpath).each do |item|
      ...
      if defined? obj.attributes[:puppets]
        item.xpath("puppets/puppet").each { |c| obj.attributes[:puppets] = c.text.strip unless c.text.strip.empty? }
        item.xpath("ports/port").each { |c| obj.attributes[:ports] = c.text.strip unless c.text.strip.empty? }
      end
